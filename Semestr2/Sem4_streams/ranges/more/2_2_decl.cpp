...
int main() {
  ...
  std::vector<Point> v;
  rng::sort(v, [](auto&& a, auto&& b) { return a.x < b.x; });
  rng::sort(v, {}, [](auto&& a) { return a.x; });
  ...
  std::transform(v.begin(), v.end(), out, [](const auto& a) { return a.x * 2; });
  rng::transform(v, out, [](auto x){ return x * 2; }, &Point::x);
}
