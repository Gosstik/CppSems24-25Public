//! There is no ; after the preprocessor directives
...
void Baz1() {
  std::cout << DEFINE_CONCAT(1, 2) << std::endl;
}
...
type min_func_##type(type a, type b) {    \
  return ((a)<(b)) ? (a) : (b);           \
}
...
type MIN_FUNC_NAME(type)(type a, type b) { \
  return SMART_MIN(a, b); \
}
