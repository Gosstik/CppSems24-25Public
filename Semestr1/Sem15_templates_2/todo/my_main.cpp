...
constexpr std::string_view GetEnumValueNameImpl() {
  std::string_view name = __PRETTY_FUNCTION__;
  name = name.substr(kEnunValueNamePrefixLen, name.size() - kEnunValueNameSuffixLen);
...
constexpr std::array<std::string_view, LimitLen> CreateArray() {
  return CreateArrayImpl<Enum, MinLimit, LimitLen>(std::make_integer_sequence<int64_t, LimitLen>{});
}
...
struct EnumeratorTraits {
 public:
  ...
  [[maybe_unused]] static constexpr std::string_view kEnumName = GetEnumNameImpl<Enum>();

  static constexpr int64_t kSignedMaxN = static_cast<int64_t>(MaxN);
  static constexpr int64_t kTypeMin = std::numeric_limits<UnderlineType>::min();
  static constexpr int64_t kTypeMax = std::numeric_limits<UnderlineType>::max();
  ...
  static constexpr int64_t kLimitLen = kMaxLimit - kMinLimit + 1; // +1 for 0 index
  ...
  static constexpr DataImpl CreateDataImpl() {
    ...
    Array<std::string_view> all_names_arr = CreateArray<Enum, kMinLimit, kLimitLen>();
 ...
 public:
  ...
  static constexpr Enum at(std::size_t i) noexcept { return Enum(kDataImpl.indexes[i]); }
